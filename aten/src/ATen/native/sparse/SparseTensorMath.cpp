...
inline SparseTensor get_result_tensor_for_unary_op(const SparseTensor& input) {
  if (c10::isIntegralType(input.scalar_type(), /*includeBool=*/true)) {
    return at::empty_like(
        input, input.options().dtype(c10::get_default_dtype()));
  }
...
// In-place log1p on uncoalesced tensors is not supported since the operation is
// not a linear map. Values of uncoalesced tensor corresponding to the same
// indices are summed and log1p(summed_value) != log1p(v1) + log1p(v2)
...
SparseTensor& log1p_out_sparse(SparseTensor& r, const SparseTensor& t) {
  ...
  if (is_same_tensor(r, t)) {
    ...
    TORCH_CHECK(
        r.is_coalesced(),
        "log1p: in-place on uncoalesced tensors is not supported");
  } else {
...
// In-place asin on uncoalesced tensors is not supported since the operation is
// not a linear map. Values of uncoalesced tensor corresponding to the same
// indices are summed and asin(summed_value) != asin(v1) + asin(v2)
...
SparseTensor& div_out_sparse_zerodim(
  ...
  TORCH_CHECK(
      value.dim() == 0,
      "Sparse division requires a scalar or ",
      "zero-dim dense tensor divisor (got shape ",
      value.sizes(),
      " for divisor)");
  TORCH_CHECK(
      !value.is_sparse(),
...
SparseTensor& floor_divide_out_sparse_zerodim(
  ...
  TORCH_CHECK(
      divisor.dim() == 0,
      "Sparse floor division requires a scalar or ",
      "zero-dim dense tensor divisor (got shape ",
      divisor.sizes(),
      " for divisor)");
  TORCH_CHECK(
...
SparseTensor& floor_divide_out_sparse_scalar(
    SparseTensor& r,
    const SparseTensor& t,
    Scalar value) {
...
Tensor norm_sparse(
  ...
  if (dim.size() > 0) {
    ...
    TORCH_CHECK(
        passed_full_reduction_check,
        "norm_sparse currently only supports full reductions, so 'dim' must either be empty or contain all dimensions of the input");
  }
  TORCH_CHECK(
      keepdim == false, "norm_sparse currently does not support keepdim=True");
  TORCH_CHECK(
      !dtype.has_value(),
      "norm_sparse currently does not support 'dtype' argument");
...
Tensor mv_sparse(const SparseTensor& self, const Tensor& vec) {
  TORCH_CHECK(
      self.ndimension() == 2 && vec.ndimension() == 1,
...
Tensor& add_out_dense_sparse_cpu(
  ...
  if (nDim > nDimI) {
    ...
    AT_DISPATCH_ALL_TYPES_AND(
        at::ScalarType::Bool, commonDtype, "add_dense_sparse", [&] {
...
SparseTensor& _sspaddmm_out_cpu(
  ...
  int64_t nnz = sparse._nnz();
  // We have to make indices contiguous as we use indices.data_ptr in _to_csr
  // which assumes row-contiguous storage
...
template <typename scalar_t>
scalar_t binary_search_strided_rightmost(
    scalar_t search_val,
    TensorAccessor<scalar_t, 1>& sorted_arr_accessor,
    int64_t sorted_arr_begin_idx,
    int64_t length,
    bool* found) {
  if (length == 0) {
    *found = false;
  ...
  while (!done_searching) {
    ...
    if (mid_val > search_val) {
      right_ind = mid_ind - 1;
        ...
        (mid_val == search_val) &&
        ((mid_ind == length - 1) ||
         (sorted_arr_accessor[sorted_arr_begin_idx + mid_ind + 1] !=
          search_val))) {
      done_searching = true;
      *found = true;
    } else {
...
Tensor& bmm_out_sparse_cpu(
  ...
  AT_DISPATCH_ALL_TYPES(values.scalar_type(), "bmm_sparse_dense", [&] {
    for (int64_t cur_mat_num = 0; (cur_mat_num < num_matrices); cur_mat_num++) {
      ...
      if (mat_end_found) {
        ...
        Tensor sparse_indices =
            indices_dim1_dim2.slice(1, mat_el_begin_idx, mat_el_end_idx);
        Tensor sparse_values =
            values.slice(0, mat_el_begin_idx, mat_el_end_idx);
